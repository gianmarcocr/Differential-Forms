...
import pygame
import pymunk
from pymunk import Vec2d, PivotJoint, SimpleMotor
...
def main():
    ...
    while running:
        for event in pygame.event.get(): # comandi tastiera e mouse
        ...
        if pygame.key.get_mods() & pygame.KMOD_SHIFT and pygame.mouse.get_pressed()[0]:
            body = pymunk.Body(10, 10)      # creates body
            body.position = mouse_pos
            shape = pymunk.Circle(body, 10, (0, 0))     # adds shape and offset wrt body.position
            shape.collision_type = COLLTYPE_BALL

            # MOTORIZED PIVOT
            PivotJoint(pymunk.Body(p,body_type="STATIC"), body, p,(0,0))
